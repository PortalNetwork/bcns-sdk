...
var PayloadCreation = function () {
  ...
  _createClass(PayloadCreation, [{
    ...
    value: async function resolutionAddress(domain, address) {
      try {
        var response = await _axios2.default.post(this.restURL + "payloadCreation/BCNS/resolution/address/" + domain + "/" + address);
        return response.data;
      } catch (error) {
        throw error.response.data;
      }
    ...
    value: async function resolutionMultihash(domain, protocol, multihash) {
      try {
        var response = await _axios2.default.post(this.restURL + "payloadCreation/BCNS/resolution/multihash/" + domain + "/" + protocol + "/" + multihash);
        return response.data;
      } catch (error) {
        throw error.response.data;
      }
